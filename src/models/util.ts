...
type StorePathRecur<M extends StorePathMap, K extends keyof M> = K extends string
  ? M[K]["collections"] extends StorePathMap
    ? StorePathIncludesNotString<M[K]["collections"]> extends string
      ? `${K}/${string}/${StorePathIncludesNotString<M[K]["collections"]>}`
      : never
    : never
  : never;

type StorePath<M extends StorePathMap> = StorePathIncludesNotString<M> extends string
  ? StorePathIncludesNotString<M>
  : never;

type ExtractFirstKey<P, M extends StorePathMap> = P extends `${infer K extends keyof M & string}/${string}/${string}`
  ? K
  : P;

type ExtractRestPath<P, K extends string, M extends StorePathMap> = P extends `${K}/${string}/${infer Rest}`
...
type ExtractZodSchemaHelper<M extends StorePathMap, P extends StorePath<M>, K extends keyof M> = K extends keyof M &
  string
  ? M[K]["collections"] extends StorePathMap
...
export const createCollectionRefFactory =
  <M extends StorePathMap>(root: DocumentReference, map: M) =>
  <P extends StorePath<M>>(path: P) => {
    const pathSegments = path.split("/");

    const zodSchema = pathSegments.reduce<{ type: "item"; value?: MappedItem } | { type: "map"; value?: StorePathMap }>(
      (prev, segment) => {
        if (prev.type === "map") {
          return {
            type: "item",
